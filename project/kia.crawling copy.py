...
conn = pymysql.connect(
    ...
    database='sknfirst'       # 접속할 데이터베이스 이름
)
print('접속성공')
...
def kia_db(faq_list):
    ...
    # 테이블이 없으면 생성 (최초 1회만 필요)
    create_sql = """
    CREATE TABLE IF NOT EXISTS faq (
        faq_id INT AUTO_INCREMENT PRIMARY KEY,
        faq_company VARCHAR(100),
    ...
    # 여러 행 저장
    insert_sql = 'INSERT INTO faq VALUES (null, %s, %s, %s, %s, %s)'
    cur.executemany(insert_sql, faq_list)  # faq_list: [(회사, 대분류, 소분류, 질문, 답변), ...]
    conn.commit()
    print('추가 완료')
